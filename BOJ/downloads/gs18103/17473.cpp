...
struct SEG {
    ...
    void init(int node, int s, int e, vector <int> &A) {
        if(s == e) {
            tree[node].mx = tree[node].orv = tree[node].andv = A[s-1];
            return;
        }
        int m = (s + e) / 2;
        init(node*2, s, m, A);
    ...
    int cal_max(int node, int s, int e, int l, int r) {
        update_lazy(node, s, e);
        if(s > r || e < l) return 0;
        if(s >= l && e <= r) return tree[node].mx;
        int m = (s + e) / 2;
